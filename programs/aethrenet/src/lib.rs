...
pub struct RegisterNode<'info> {
    ...
    #[account(
        init,
        payer = authority,
        space = 8 + 32 + 4 + max_uri,
    )]
    pub nodeDevice : Account<'info , NodeDevice>,
    ...
    pub networkStats : Account<'info , NetworkStats>,
...
pub struct DeregisterNode<'info> {
    ...
    #[account(
        mut , seeds = [b"network-stats"] , bump
    )]
    pub networkStats : Account<'info , NetworkStats>,
...
pub mod aethernet{
    ...
    pub fn register_node(ctx : Context<RegisterNode> , uri : String) -> Result<()>{
        require!(uri.len() <= max_uri , AethernetError::UriTooLong);
        ...
        let node_device = &mut ctx.accounts.nodeDevice;
        node_device.authority = ctx.accounts.authority.key();
        node_device.uri = uri;

        let network_stats = &mut ctx.accounts.networkStats;
        network_stats.total_nodes += 1;
        Ok(())
    }
    ...
    pub fn deregister_node(ctx: Context<DeregisterNode>) -> Result<()>{
        ...
        let stats = &mut ctx.accounts.networkStats;
        if stats.total_nodes > 0{
            stats.total_nodes -= 1;
        }
